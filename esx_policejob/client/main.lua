	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = _U('comserv'),			value = 'comserv'},
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						TriggerServerEvent('esx_policejob:requestarrest', GetPlayerServerId(ESX.Game.GetClosestPlayer()), GetEntityHeading(GetPlayerPed(-1)), GetEntityCoords(GetPlayerPed(-1)), GetEntityForwardVector(PlayerPedId()))
					elseif action == 'uncuff' then
						TriggerServerEvent('esx_policejob:requestrelease', GetPlayerServerId(ESX.Game.GetClosestPlayer()), GetEntityHeading(GetPlayerPed(-1)),GetEntityCoords(GetPlayerPed(-1)), GetEntityForwardVector(PlayerPedId()))
					...
					elseif action == 'comserv' then
						ComServPlayer(GetPlayerServerId(closestPlayer))
...
function ComServPlayer(player)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'comserv_menu', {
		title = _U('comserv_menu_info'),
	}, function (data2, menu)
		local servTime = tonumber(data2.value)
		if servTime == nil then
			ESX.ShowNotification('invalid number!')
		else
			TriggerServerEvent("esx_communityservice:sendToCommunityService", player, servTime)
			menu.close()
		end
	end, function (data2, menu)
		menu.close()
...
Citizen.CreateThread(function()
	while true do
		...
		if isHandcuffed then
			...
			DisableControlAction(0, 243, true) -- ` (Below ESC)
